...
local VERSION = "11.3.0" -- Corresponds to the original Vimscript version
...
local state = {
  ...
  config = { -- Default configuration
    ...
    plugin_name = "neovim-wakatime", -- Differentiate from vim plugin
  },
  home = "",
  plugin_root_folder = "",
  config_file = "",
...
executable = function(path)
  return path and path ~= "" and fn.executable(path) == 1
end

contains = function(str, substr)
  if not str or not substr or str == "" or substr == "" then
    return false
  end
...
get_python_binary = function()
  if state.config.python_binary and executable(state.config.python_binary) then
    return state.config.python_binary
  end
...
setup_debug_mode = function()
  if not state.debug_mode_already_setup then
    ...
    if state.is_debug_on then
      vim.notify("[WakaTime] Debug mode enabled.", vim.log.levels.INFO)
    end
  end
end
...
setup_config_file = function()
  if not state.config_file_already_setup then
    ...
    if api_key == "" then
      api_key = get_ini_setting("settings", "apikey")
    end -- Legacy key name
    ...
    if not found_api_key then
      ...
      if vault_cmd and vault_cmd ~= "" then
        ...
        if handle then
          ...
          handle:close()
          if vault_key and chomp(vault_key) ~= "" then
            found_api_key = true
          end
        end
      end
...
local function neovim_async_install_output_handler(job_id, data, event)
  if data then
    table.insert(state.nvim_async_output, table.concat(data, "\n"))
  end
end
  ...
  if not state.cli_already_setup then
    ...
    if cli_path and executable(cli_path) then
      ...
      if executable("wakatime-cli") then
        ...
        if state.is_debug_on then
          vim.notify("[WakaTime] Using wakatime-cli found in PATH", vim.log.levels.DEBUG)
        end
      elseif
        executable("wakatime")
      ...
      then
        ...
        state.autoupdate_cli = false
        if state.is_debug_on then
          vim.notify("[WakaTime] Using legacy wakatime found in PATH", vim.log.levels.DEBUG)
        end
      elseif not state.is_windows and fn.filereadable("/usr/local/bin/wakatime-cli") == 1 then
        ...
        state.autoupdate_cli = false
        if state.is_debug_on then
          vim.notify("[WakaTime] Using Homebrew wakatime-cli", vim.log.levels.DEBUG)
        end
      else
...
send_heartbeats = function()
  ...
  if job_id and job_id > 0 then
    if extra_heartbeats_json ~= "" then
      fn.chansend(job_id, extra_heartbeats_json .. "\n")
    end
...
local function prompt_for_api_key()
  ...
  if current_key == "" then
    current_key = get_ini_setting("settings", "apikey")
  end
...
local function run_cli_command(args, output_buffer_key, callback_key, exit_handler)
  ...
  fn.jobstart(cmd_args, {
    on_stdout = function(job_id, data, event)
      if data then
        table.insert(state[output_buffer_key], table.concat(data))
      end
    end,
...
print_today = function(msg)
  vim.notify("Today: " .. (msg or "No data"), vim.log.levels.INFO)
end
print_file_expert = function(msg)
  local output = msg or ""
  if output == "" then
    output = "No expert data found for this file."
  end
...
function M.setup(user_config)
  ...
  api.nvim_create_autocmd({ "BufEnter", "VimEnter" }, {
    ...
    pattern = "*",
    callback = function()
      init_and_handle_activity(false)
    end,
  })
  api.nvim_create_autocmd({ "CursorHold", "CursorHoldI" }, {
    ...
    pattern = "*",
    callback = function()
      handle_activity(false)
    end, -- Don't re-run full init on cursor hold
  })
  ...
  api.nvim_create_user_command("WakaTimeDebugEnable", function()
    set_debug_mode(true)
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeDebugDisable", function()
    set_debug_mode(false)
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeScreenRedrawDisable", function()
    set_redraw_setting("disabled")
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeScreenRedrawEnable", function()
    set_redraw_setting("enabled")
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeScreenRedrawEnableAuto", function()
    set_redraw_setting("auto")
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeToday", function()
    M.get_today_summary()
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeFileExpert", function()
    M.get_file_experts()
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeCliLocation", function()
    M.get_cli_location()
  end, { nargs = 0 })
  api.nvim_create_user_command("WakaTimeCliVersion", function()
    M.get_cli_version()
  end, { nargs = 0 })
